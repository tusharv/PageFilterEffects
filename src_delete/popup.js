...
function applyStyle(event=null){
	...
	chrome.scripting.executeScript(null,{code:"document.body.style.transition='filter 3s ease-out';document.body.style.filter='" + config + "'"});
}

function resetStyle(){
	chrome.scripting.executeScript(null,{code:"document.body.style.filter='none'"});
